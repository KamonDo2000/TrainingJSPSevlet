...
<head>
	...
	<c:set var="searchPath" value="${contextPath}/dichvu/search" />
	<c:set var="paggingPath" value="${contextPath}/dichvu/index?" />
            ...
            <header class="row bg-head py-2 px-3 m-0 align-items-center">
                ...
                <nav class="col-4 d-flex justify-content-center align-items-center">
                    <button id="add-btn" type="button" class="btn btn-success ml-3">
                        <i class="fa-solid fa-circle-plus"></i> Add New Dịch Vụ
                    </button>
                </nav>
            </header>
                ...
                <div class="table-responsive px-3 tableFixHead">
                    <table class="table table-striped">
                        ...
                        <tbody id="idTable" class="form">
                            <c:forEach var="e" items="${DataDichVu}" varStatus="loop">
                                <tr>
									<td hidden="true">${loop.count}</td>
									<td class="txt-MaDV">${e.getMaDV()}</td>
									<td class="txt-TenDV">${e.getTenDV()}</td>
									<td class="txt-DonGia">${e.getDonGia()}</td>
									<td class="txt-DonViTinh">${e.getDonViTinh()}</td>
                                    <td>
